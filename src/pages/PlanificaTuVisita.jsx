    ...
    <div>
          ...
          <div className="grid-2" style={{ gap: '3rem', alignItems: 'start' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                {serviceInfo.map((item, i) => (
                  <div key={i} style={{
                    ...
                    background: '#fff', borderRadius: '0.85rem', padding: '1rem 1.25rem',
                    boxShadow: '0 1px 6px rgba(15,23,42,0.07)',
                    border: '1px solid rgba(15,23,42,0.05)',
                  }}>
                    <span style={{ fontSize: '1.4rem', flexShrink: 0 }}>{item.icon}</span>
                    <div>
                      <div style={{ fontWeight: 700, fontSize: '0.75rem', color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.06em' }}>{item.label}</div>
                      <div style={{ color: '#0f172a', fontWeight: 500 }}>{item.val}</div>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="animate-on-scroll slide-right">
              ...
              <div style={{
                ...
                boxShadow: '0 12px 40px rgba(21,40,71,0.25)',
              }}>
                <h4 style={{ color: '#e8bf6a', marginBottom: '1rem' }}>📍 Dirección</h4>
                <p style={{ color: 'rgba(255,255,255,0.88)', lineHeight: 1.75, marginBottom: 0 }}>
      ...
      <section className="section" style={{
        ...
        <div style={{
          ...
          background: 'radial-gradient(ellipse at 20% 50%, rgba(201,153,59,0.1) 0%, transparent 55%)',
          pointerEvents: 'none',
        }} />
        <div className="container" style={{ position: 'relative' }}>
          <div className="grid-2" style={{ gap: '3rem', alignItems: 'center' }}>
            <div className="animate-on-scroll slide-left">
              ...
              <p style={{ color: 'rgba(255,255,255,0.82)', lineHeight: 1.8, marginBottom: '1rem' }}>
              ...
              <ul style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                {['Cuna (0–2 años)', 'Preescolares (3–5 años)', 'Primaria (6–9 años)', 'Pre-adolescentes (10–12 años)'].map((g, i) => (
                  <li key={i} style={{ color: 'rgba(255,255,255,0.82)', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                    <span style={{ color: '#e8bf6a' }}>✓</span> {g}
                  </li>
                ))}
              </ul>
            </div>
            <div className="animate-on-scroll slide-right" style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              {[
                ...
                <div key={i} style={{
                  background: 'rgba(255,255,255,0.07)', borderRadius: '1rem',
                  ...
                  <div>
                    <h5 style={{ color: '#e8bf6a', marginBottom: '0.25rem' }}>{item.title}</h5>
                    <p style={{ color: 'rgba(255,255,255,0.7)', fontSize: '0.9rem', marginBottom: 0 }}>{item.desc}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div style={{ maxWidth: '800px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            {faqs.map((faq, i) => (
              <div key={i} className={`animate-on-scroll delay-${(i % 3 + 1) * 100}`} style={{
                ...
                padding: '1.5rem', boxShadow: '0 2px 10px rgba(15,23,42,0.07)',
                border: '1px solid rgba(15,23,42,0.06)',
              }}>
                <h5 style={{ marginBottom: '0.6rem', color: 'var(--color-navy)', display: 'flex', alignItems: 'flex-start', gap: '0.5rem' }}>
                  <span style={{ color: '#c9993b', flexShrink: 0, fontFamily: 'var(--font-heading)' }}>?</span>
                  {faq.q}
                </h5>
